...
public class AnvilThrowSkill extends AnvilDropSkill {
			...
			DamageDisplayListener damageDisplayListener) 
	{
		...
		if (!targets.isEmpty())
		{
			...
			caster.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, caster.getEyeLocation(), 30, 2.0, 2.0, 2.0);
			caster.getWorld().spawnParticle(Particle.FLAME, caster.getEyeLocation(), 30, 2.0, 2.0, 2.0);
		...
		{
			FallingBlock anvil = (FallingBlock) target.getWorld().spawnFallingBlock(target.getLocation(), Bukkit.createBlockData(Material.DAMAGED_ANVIL));
			...
			anvil.setVelocity(MathUtil.calculateArcBetweenPoints(caster.getLocation().getDirection(), target.getLocation().getDirection(), 
					(int) (caster.getHeight() * 2)).normalize());
